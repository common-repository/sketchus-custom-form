...
add_filter( 'skfom_spam', 'skfom_akismet' );

function skfom_akismet( $spam ) {
	if ( $spam ) {
		return $spam;
	}

	if ( ! skfom_akismet_is_available() ) {
		return false;
	}

	if ( ! $params = skfom_akismet_submitted_params() ) {
		return false;
	}
	...
	return skfom_akismet_comment_check( $c );
}

function skfom_akismet_is_available() {
...
function skfom_akismet_submitted_params() {
	$params = array(
		'author' => '',
		'author_email' => '',
		'author_url' => '',
	...
	foreach ( (array) $_POST as $key => $val ) {
		if ( '_skfom' == substr( $key, 0, 6 ) || '_wpnonce' == $key ) {
			continue;
		}

		if ( is_array( $val ) ) {
			$val = implode( ', ', skfom_array_flatten( $val ) );
		}
		...
		if ( $tags = skfom_scan_shortcode( array( 'name' => $key ) ) ) {
			$tag = $tags[0];
...
function skfom_akismet_comment_check( $comment ) {
	global $akismet_api_host, $akismet_api_port;

	$spam = false;
	$query_string = skfom_build_query( $comment );
	...
	return apply_filters( 'skfom_akismet_comment_check', $spam, $comment );
}
