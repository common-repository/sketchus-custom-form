(function($) {
	...
	$('form.tag-generator-panel .control-box :input').change(function(event) {
		...
		_skfom.taggen.normalize($(this));
		_skfom.taggen.update(form);
	});

	$('input.insert-tag').click(function(event) {
		var form = $(this).closest('form.tag-generator-panel');
		var tag = form.find('input.tag').val();
		_skfom.taggen.insert(tag);
		tb_remove(); // close thickbox
		return false;
	});

	_skfom.taggen.update = function($form) {
		...
		if ($.isFunction(_skfom.taggen.update[id])) {
			return _skfom.taggen.update[id].call(this, $form);
		}
		...
		$form.find('input.tag').each(function() {
			...
			components = _skfom.taggen.compose(tag_type, $form);
			$(this).val(components);
		});
	...
	_skfom.taggen.update.captcha = function($form) {
		var captchac = _skfom.taggen.compose('captchac', $form);
		var captchar = _skfom.taggen.compose('captchar', $form);

		$form.find('input.tag').val(captchac + ' ' + captchar);
	};

	_skfom.taggen.compose = function(tagType, $form) {
	...
	_skfom.taggen.normalize = function($input) {
		var val = $input.val();
		...
		if ($input.is(':checkbox.exclusive')) {
			_skfom.taggen.exclusiveCheckbox($input);
		}
	}

	_skfom.taggen.exclusiveCheckbox = function($cb) {
		if ($cb.is(':checked')) {
			$cb.siblings(':checkbox.exclusive').prop('checked', false);
		}
	};

	_skfom.taggen.insert = function(content) {
		$('textarea#skfom-form').each(function() {
			this.focus();
