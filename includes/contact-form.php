...
class SKFOM_ContactForm {

	const post_type = 'skfom_contact_form';
	...
	public static function register_post_type() {
		register_post_type( self::post_type, array(
			'labels' => array(
				'name' => __( 'Contact Forms', 'sketchus-custom-form' ),
				'singular_name' => __( 'Contact Form', 'sketchus-custom-form' ) ),
			'rewrite' => false,
			'query_var' => false ) );
	}
	...
	public static function get_template( $args = '' ) {
		...
		if ( $locale ) {
			$mo_orig = $l10n['sketchus-custom-form'];
			skfom_load_textdomain( $locale );
		}

		self::$current = $contact_form = new self;
		$contact_form->title =
			( $title ? $title : __( 'Untitled', 'sketchus-custom-form' ) );
		$contact_form->locale = ( $locale ? $locale : get_locale() );

		$properties = $contact_form->get_properties();

		foreach ( $properties as $key => $value ) {
			$properties[$key] = SKFOM_ContactFormTemplate::get_default( $key );
		}

		$contact_form->properties = $properties;

		$contact_form = apply_filters( 'skfom_contact_form_default_pack',
			$contact_form, $args );

		if ( isset( $mo_orig ) ) {
			$l10n['sketchus-custom-form'] = $mo_orig;
		}
	...
	private static function get_unit_tag( $id = 0 ) {
		...
		if ( in_the_loop() ) {
			$unit_tag = sprintf( 'skfom-f%1$d-p%2$d-o%3$d',
				absint( $id ), get_the_ID(), $global_count );
		} else {
			$unit_tag = sprintf( 'skfom-f%1$d-o%2$d',
	...
	private function __construct( $post = null ) {
		...
		do_action( 'skfom_contact_form', $this );
	}

	public function __get( $name ) {
		$message = __( '<code>%1$s</code> property of a <code>SKFOM_ContactForm</code> object is <strong>no longer accessible</strong>. Use <code>%2$s</code> method instead.', 'sketchus-custom-form' );
	...
	public function get_properties() {
		...
		$properties = (array) apply_filters( 'skfom_contact_form_properties',
			$properties, $this );
	...
	public function set_title( $title ) {
		...
		if ( '' === $title ) {
			$title = __( 'Untitled', 'sketchus-custom-form' );
		}
	...
	public function is_posted() {
		if ( ! SKFOM_Submission::get_instance() ) {
			return false;
		}

		if ( empty( $_POST['_skfom_unit_tag'] ) ) {
			return false;
		}

		return $this->unit_tag == $_POST['_skfom_unit_tag'];
	}
	...
	public function form_html( $args = '' ) {
		...
		$url = apply_filters( 'skfom_form_action_url', $url );

		$id_attr = apply_filters( 'skfom_form_id_attr',
			preg_replace( '/[^A-Za-z0-9:._-]/', '', $args['html_id'] ) );

		$name_attr = apply_filters( 'skfom_form_name_attr',
		...
		$class = implode( ' ', $class );
		$class = apply_filters( 'skfom_form_class_attr', $class );

		$enctype = apply_filters( 'skfom_form_enctype', '' );

		$novalidate = apply_filters( 'skfom_form_novalidate', skfom_support_html5() );

		$atts = array(
			'action' => esc_url( $url ),
			'method' => 'post',
			'class' => $class,
			'enctype' => skfom_enctype_value( $enctype ),
	...
	private function form_hidden_fields() {
		$hidden_fields = array(
			'_skfom' => $this->id,
			'_skfom_version' => SKFOM_VERSION,
			'_skfom_locale' => $this->locale,
			'_skfom_unit_tag' => $this->unit_tag );

		if ( SKFOM_VERIFY_NONCE ) {
			$hidden_fields['_wpnonce'] = skfom_create_nonce( $this->id );
		}

		$hidden_fields += (array) apply_filters(
			'skfom_form_hidden_fields', array() );
	...
	public function form_response_output() {
		...
		if ( $this->is_posted() ) { // Post response output for non-AJAX
			...
			if ( $submission->is( 'validation_failed' ) ) {
				$class .= ' skfom-validation-errors';
			} elseif ( $submission->is( 'spam' ) ) {
				$class .= ' skfom-spam-blocked';
			} elseif ( $submission->is( 'mail_sent' ) ) {
				$class .= ' skfom-mail-sent-ok';
			} elseif ( $submission->is( 'mail_failed' ) ) {
				$class .= ' skfom-mail-sent-ng';
			}
		} else {
			$class .= ' skfom-display-none';
		}
		...
		$output = apply_filters( 'skfom_form_response_output',
	...
	public function validation_error( $name ) {
		...
		$error = sprintf(
			'<span role="alert" class="skfom-not-valid-tip">%s</span>',
			esc_html( $error ) );

		return apply_filters( 'skfom_validation_error', $error, $name, $this );
	}

	/* Form Elements */

	public function form_do_shortcode() {
		$manager = SKFOM_ShortcodeManager::get_instance();
		$form = $this->prop( 'form' );

		if ( SKFOM_AUTOP ) {
			$form = $manager->normalize_shortcode( $form );
			$form = skfom_autop( $form );
		}
	...
	public function form_elements() {
		return apply_filters( 'skfom_form_elements', $this->form_do_shortcode() );
	}
	...
	public function collect_mail_tags( $args = '' ) {
		...
		return apply_filters( 'skfom_collect_mail_tags', $mailtags );
	}
	...
	public function submit( $ajax = false ) {
		$submission = SKFOM_Submission::get_instance( $this );
		...
		if ( $ajax ) {
			...
			if ( ! empty( $on_submit ) ) {
				$result['scripts_on_submit'] = array_map(
					'skfom_strip_quote', $on_submit );
			}
		}

		do_action( 'skfom_submit', $this, $result );
	...
	public function message( $status, $filter = true ) {
		...
		if ( $filter ) {
			...
			$message = skfom_mail_replace_tags( $message, array( 'html' => true ) );
			$message = apply_filters( 'skfom_display_message', $message, $status );
		}
		...
		if ( $post_id ) {
			foreach ( $props as $prop => $value ) {
				update_post_meta( $post_id, '_' . $prop,
					skfom_normalize_newline_deep( $value ) );
			}

			if ( skfom_is_valid_locale( $this->locale ) ) {
				update_post_meta( $post_id, '_locale', $this->locale );
			}

			if ( $this->initial() ) {
				$this->id = $post_id;
				do_action( 'skfom_after_create', $this );
			} else {
				do_action( 'skfom_after_update', $this );
			}

			do_action( 'skfom_after_save', $this );
		}
	...
	public function copy() {
		...
		return apply_filters( 'skfom_copy', $new, $this );
	}
	...
	public function shortcode( $args = '' ) {
		...
		if ( $args['use_old_format'] ) {
			...
			$shortcode = sprintf( '[sketchus-custom-form id="%1$d" title="%2$s"]',
				$this->id, $title );
		}

		return apply_filters( 'skfom_contact_form_shortcode', $shortcode, $args, $this );
	}

	public function validate_configuration() {
		if ( ! $this->initial() ) {
			if ( ! $this->config_validator ) {
				$this->config_validator = new SKFOM_ConfigValidator( $this );
			}
...
function skfom_contact_form( $id ) {
	return SKFOM_ContactForm::get_instance( $id );
}

function skfom_get_contact_form_by_old_id( $old_id ) {
	global $wpdb;
	...
	if ( $new_id = $wpdb->get_var( $q ) ) {
		return skfom_contact_form( $new_id );
	}
}

function skfom_get_contact_form_by_title( $title ) {
	$page = get_page_by_title( $title, OBJECT, SKFOM_ContactForm::post_type );

	if ( $page ) {
		return skfom_contact_form( $page->ID );
	}

	return null;
}

function skfom_get_current_contact_form() {
	if ( $current = SKFOM_ContactForm::get_current() ) {
		return $current;
	}
}

function skfom_is_posted() {
...
function skfom_get_hangover( $name, $default = null ) {
	if ( ! skfom_is_posted() ) {
		return $default;
	}
...
function skfom_get_validation_error( $name ) {
	if ( ! $contact_form = skfom_get_current_contact_form() ) {
		return '';
	}
...
function skfom_get_message( $status ) {
	if ( ! $contact_form = skfom_get_current_contact_form() ) {
		return '';
	}
...
function skfom_scan_shortcode( $cond = null ) {
	if ( ! $contact_form = skfom_get_current_contact_form() ) {
		return array();
	}
...
function skfom_form_controls_class( $type, $default = '' ) {
	$type = trim( $type );
	$default = array_filter( explode( ' ', $default ) );

	$classes = array_merge( array( 'skfom-form-control' ), $default );

	$typebase = rtrim( $type, '*' );
	$required = ( '*' == substr( $type, -1 ) );

	$classes[] = 'skfom-' . $typebase;

	if ( $required ) {
		$classes[] = 'skfom-validates-as-required';
	}
...
function skfom_contact_form_tag_func( $atts, $content = null, $code = '' ) {
	if ( is_feed() ) {
		return '[sketchus-custom-form]';
	}

	if ( 'sketchus-custom-form' == $code ) {
		...
		if ( ! $contact_form = skfom_contact_form( $id ) ) {
			$contact_form = skfom_get_contact_form_by_title( $title );
		}
		...
		$contact_form = skfom_get_contact_form_by_old_id( $id );
	}

	if ( ! $contact_form ) {
		return '[sketchus-custom-form 404 "Not Found"]';
	}
