...
class SKFOM_Mail {
	...
	private function compose( $send = true ) {
		...
		$components = apply_filters( 'skfom_mail_components',
			$components, skfom_get_current_contact_form(), $this );

		$subject = skfom_strip_newline( $components['subject'] );
		$sender = skfom_strip_newline( $components['sender'] );
		$recipient = skfom_strip_newline( $components['recipient'] );
		...
		if ( $use_html ) {
			...
			$headers .= "X-SKFOM-Content-Type: text/html\n";
		} else {
			$headers .= "X-SKFOM-Content-Type: text/plain\n";
		}
	...
	public function replace_tags( $content, $html = false ) {
		...
		return skfom_mail_replace_tags( $content, $args );
	}
...
function skfom_mail_replace_tags( $content, $args = '' ) {
	...
	if ( is_array( $content ) ) {
		foreach ( $content as $key => $value ) {
			$content[$key] = skfom_mail_replace_tags( $value, $args );
		}
	...
	foreach ( $content as $num => $line ) {
		$line = new SKFOM_MailTaggedText( $line, $args );
...
add_action( 'phpmailer_init', 'skfom_phpmailer_init' );

function skfom_phpmailer_init( $phpmailer ) {
	$skfom_content_type = false;

	foreach ( (array) $phpmailer->getCustomHeaders() as $custom_header ) {
		if ( 'X-SKFOM-Content-Type' == $custom_header[0] ) {
			$skfom_content_type = trim( $custom_header[1] );
			break;
		}
	}

	if ( 'text/html' == $skfom_content_type ) {
		$phpmailer->msgHTML( $phpmailer->Body );
	}
}

class SKFOM_MailTaggedText {
	...
	private function replace_tags_callback( $matches, $html = false ) {
		...
		if ( ! empty( $values ) ) {
			...
			$values = skfom_strip_quote_deep( $matches[0] );
		}
		...
		if ( null !== $submitted ) {
			...
			$replaced = skfom_flat_join( $replaced );
			...
			$replaced = apply_filters( 'skfom_mail_tag_replaced',
		...
		$special = apply_filters( 'skfom_special_mail_tags', '', $tagname, $html );
...
add_filter( 'skfom_special_mail_tags', 'skfom_special_mail_tag', 10, 3 );

function skfom_special_mail_tag( $output, $name, $html ) {
	$name = preg_replace( '/^skfom\./', '_', $name ); // for back-compat
	...
	if ( '_post_' == substr( $name, 0, 6 ) ) {
		...
		if ( $unit_tag
		&& preg_match( '/^skfom-f(\d+)-p(\d+)-o(\d+)$/', $unit_tag, $matches ) ) {
