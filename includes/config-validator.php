...
class SKFOM_ConfigValidator {
	...
	public function get_error_message( $section ) {
		...
		switch ( $code ) {
			case self::error_maybe_empty:
				return __( "This field can be empty depending on user input.", 'sketchus-custom-form' );
			case self::error_invalid_syntax:
				return __( "This field has syntax errors.", 'sketchus-custom-form' );
			case self::error_email_not_in_site_domain:
				return __( "This email address does not belong to the same domain as the site.", 'sketchus-custom-form' );
			case self::error_html_in_message:
				return __( "HTML tags are not allowed in a message.", 'sketchus-custom-form' );
			default:
				return '';
		}
	}
	...
	public function validate_mail( $template = 'mail' ) {
		...
		$subject = new SKFOM_MailTaggedText( $subject,
			array( 'callback' => $callback ) );
		$subject = $subject->replace_tags();
		$subject = skfom_strip_newline( $subject );
		...
		$sender = new SKFOM_MailTaggedText( $sender,
			array( 'callback' => $callback ) );
		$sender = $sender->replace_tags();
		$sender = skfom_strip_newline( $sender );

		if ( ! skfom_is_mailbox_list( $sender ) ) {
			$this->add_error( sprintf( '%s.sender', $template ),
				self::error_invalid_syntax );
		} elseif ( ! skfom_is_email_in_site_domain( $sender ) ) {
			$this->add_error( sprintf( '%s.sender', $template ),
				self::error_email_not_in_site_domain );
		}

		$recipient = $components['recipient'];
		$recipient = new SKFOM_MailTaggedText( $recipient,
			array( 'callback' => $callback ) );
		$recipient = $recipient->replace_tags();
		$recipient = skfom_strip_newline( $recipient );

		if ( ! skfom_is_mailbox_list( $recipient ) ) {
			$this->add_error( sprintf( '%s.recipient', $template ),
				self::error_invalid_syntax );
		}

		$additional_headers = $components['additional_headers'];
		$additional_headers = new SKFOM_MailTaggedText( $additional_headers,
			array( 'callback' => $callback ) );
		...
		$body = new SKFOM_MailTaggedText( $body,
			array( 'callback' => $callback ) );
	...
	public function test_additional_headers_syntax( $content ) {
		...
		foreach ( $headers as $header ) {
			...
			&& ! skfom_is_mailbox_list( $matches[2] ) ) {
				return false;
			}
		}
	...
	public function replace_mail_tags_with_minimum_input( $matches ) {
		...
		if ( $form_tags ) {
			$form_tag = new SKFOM_Shortcode( $form_tags[0] );
			...
			if ( $is_selectable ) {
				if ( $form_tag->pipes instanceof SKFOM_Pipes ) {
					if ( $do_not_heat ) {
				...
				if ( $last_item && skfom_is_mailbox_list( $last_item ) ) {
			...
			$tagname = preg_replace( '/^skfom\./', '_', $tagname ); // for back-compat
