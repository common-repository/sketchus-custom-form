...
class SKFOM_Submission {
	...
	public static function get_instance( SKFOM_ContactForm $contact_form = null ) {
	...
	private function setup_posted_data() {
		...
		$this->posted_data = apply_filters( 'skfom_posted_data', $posted_data );
	...
	private function submit() {
		...
		$this->meta = array(
			...
			'url' => preg_replace( '%(?<!:|/)/.*$%', '',
				untrailingslashit( home_url() ) ) . skfom_get_request_uri(),
			'timestamp' => current_time( 'timestamp' ),
			'unit_tag' => isset( $_POST['_skfom_unit_tag'] )
				? $_POST['_skfom_unit_tag'] : '' );
		...
		if ( ! $this->validate() ) { // Validation error occured
			...
			do_action( 'skfom_mail_sent', $contact_form );

		} else {
			$this->status = 'mail_failed';
			$this->response = $contact_form->message( 'mail_sent_ng' );

			do_action( 'skfom_mail_failed', $contact_form );
		}
	...
	private function validate() {
		...
		require_once SKFOM_PLUGIN_DIR . '/includes/validation.php';
		$result = new SKFOM_Validation();

		$tags = $this->contact_form->form_scan_shortcode();

		foreach ( $tags as $tag ) {
			$result = apply_filters( 'skfom_validate_' . $tag['type'],
				$result, $tag );
		}

		$result = apply_filters( 'skfom_validate', $result, $tags );
	...
	private function accepted() {
		return apply_filters( 'skfom_acceptance', true );
	}
	...
	private function spam() {
		...
		if ( SKFOM_VERIFY_NONCE && ! $this->verify_nonce() ) {
			$spam = true;
		}
		...
		return apply_filters( 'skfom_spam', $spam );
	}

	private function verify_nonce() {
		return skfom_verify_nonce( $_POST['_wpnonce'], $this->contact_form->id() );
	}

	private function blacklist_check() {
		$target = skfom_array_flatten( $this->posted_data );
		$target[] = $this->get_meta( 'remote_ip' );
		$target[] = $this->get_meta( 'user_agent' );

		$target = implode( "\n", $target );

		return skfom_blacklist_check( $target );
	}
	...
	private function mail() {
		...
		do_action( 'skfom_before_send_mail', $contact_form );

		$skip_mail = $this->skip_mail || ! empty( $contact_form->skip_mail );
		$skip_mail = apply_filters( 'skfom_skip_mail', $skip_mail, $contact_form );

		if ( $skip_mail ) {
			return true;
		}

		$result = SKFOM_Mail::send( $contact_form->prop( 'mail' ), 'mail' );
		...
		if ( $result ) {
			...
			$additional_mail = apply_filters( 'skfom_additional_mail',
				$additional_mail, $contact_form );

			foreach ( $additional_mail as $name => $template ) {
				SKFOM_Mail::send( $template, $name );
			}
