...
function skfom_is_valid_locale( $locale ) {
	$pattern = '/^[a-z]{2,3}(?:_[a-zA-Z_]{2,})?$/';
	return (bool) preg_match( $pattern, $locale );
}

function skfom_is_rtl( $locale = '' ) {
	if ( empty( $locale ) ) {
...
function skfom_load_textdomain( $locale = null ) {
	global $l10n;

	$domain = 'sketchus-custom-form';
