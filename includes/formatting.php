...
function skfom_autop( $pee, $br = 1 ) {
	...
	/* skfom: remove select and input */
	$allblocks = '(?:table|thead|tfoot|caption|col|colgroup|tbody|tr|td|th|div|dl|dd|dt|ul|ol|li|pre|form|map|area|blockquote|address|math|style|p|h[1-6]|hr|fieldset|legend|section|article|aside|hgroup|header|footer|nav|figure|figcaption|details|menu|summary)';
	$pee = preg_replace( '!(<' . $allblocks . '[^>]*>)!', "\n$1", $pee );
	$pee = preg_replace( '!(</' . $allblocks . '>)!', "$1\n\n", $pee );

	/* skfom: take care of [response] and [recaptcha] tag */
...
function skfom_sanitize_query_var( $text ) {
...
function skfom_strip_quote( $text ) {
	$text = trim( $text );
...
function skfom_strip_quote_deep( $arr ) {
	if ( is_string( $arr ) )
		return skfom_strip_quote( $arr );

	if ( is_array( $arr ) ) {
		$result = array();

		foreach ( $arr as $key => $text )
			$result[$key] = skfom_strip_quote_deep( $text );

		return $result;
	}
}

function skfom_normalize_newline( $text, $to = "\n" ) {
	if ( ! is_string( $text ) )
		return $text;
...
function skfom_normalize_newline_deep( $arr, $to = "\n" ) {
	if ( is_array( $arr ) ) {
		$result = array();

		foreach ( $arr as $key => $text )
			$result[$key] = skfom_normalize_newline_deep( $text, $to );

		return $result;
	}

	return skfom_normalize_newline( $arr, $to );
}

function skfom_strip_newline( $str ) {
	$str = (string) $str;
	$str = str_replace( array( "\r", "\n" ), '', $str );
	return trim( $str );
}

function skfom_canonicalize( $text ) {
 ...
 */
function skfom_is_name( $string ) {
	return preg_match( '/^[A-Za-z][-A-Za-z0-9_:.]*$/', $string );
}

function skfom_sanitize_unit_tag( $tag ) {
	$tag = preg_replace( '/[^A-Za-z0-9_-]/', '', $tag );
	return $tag;
}

function skfom_is_email( $email ) {
	$result = is_email( $email );
	return apply_filters( 'skfom_is_email', $result, $email );
}

function skfom_is_url( $url ) {
	$result = ( false !== filter_var( $url, FILTER_VALIDATE_URL ) );
	return apply_filters( 'skfom_is_url', $result, $url );
}

function skfom_is_tel( $tel ) {
	$result = preg_match( '/^[+]?[0-9() -]*$/', $tel );
	return apply_filters( 'skfom_is_tel', $result, $tel );
}

function skfom_is_number( $number ) {
	$result = is_numeric( $number );
	return apply_filters( 'skfom_is_number', $result, $number );
}

function skfom_is_date( $date ) {
	$result = preg_match( '/^([0-9]{4,})-([0-9]{2})-([0-9]{2})$/', $date, $matches );

	if ( $result )
		$result = checkdate( $matches[2], $matches[3], $matches[1] );

	return apply_filters( 'skfom_is_date', $result, $date );
}

function skfom_is_mailbox_list( $mailbox_list ) {
...
function skfom_is_email_in_domain( $email, $domain ) {
	$email_list = skfom_is_mailbox_list( $email );
...
function skfom_is_email_in_site_domain( $email ) {
	...
	if ( skfom_is_email_in_domain( $email, $site_domain ) ) {
		return true;
	}
	...
	if ( preg_match( '%^https?://([^/]+)%', home_url(), $matches ) ) {
		...
		&& skfom_is_email_in_domain( $email, $site_domain ) ) {
			return true;
		}
	}
...
function skfom_antiscript_file_name( $filename ) {
...
function skfom_mask_password( $text, $length_unmasked = 0 ) {
	$length = strlen( $text );
